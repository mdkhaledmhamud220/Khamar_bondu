...
const getCowDetails = (cow, vaccines = [], medicines = []) => {
  ...
  const weightDelta =
    expectedWeightKg > 0
      ? Math.abs(expectedWeightKg - latestWeightKg) / expectedWeightKg
      : 0;

  const vaccination = Math.max(
    0,
    30 - overdueVaccines * 6 - pendingVaccines * 2,
  );
  const medicine = Math.max(0, 25 - recentMedicines * 4);
  const weight =
    expectedWeightKg > 0 ? Math.max(0, 25 - Math.round(weightDelta * 25)) : 15;
  const age =
    ageMonths > 0 ? Math.min(10, Math.max(4, Math.round(ageMonths / 6))) : 4;
  ...
  const healthScore = Math.max(
    0,
    Math.min(100, vaccination + medicine + weight + age + feeding),
  );

  return {
    healthScore: cow.health_score || healthScore,
    healthGrade: cow.health_score
      ? gradeFromScore(cow.health_score)
      : gradeFromScore(healthScore),
    healthDetails,
  };
};
      ...
      <View style={styles.modal}>
        ...
        >
          {cow && (
            <View style={styles.cowInfoBox}>
              ...
              <View>
                ...
                <Text style={styles.cowInfoSub}>
                  Health Score: {cow.health_score} • {cow.healthGrade}
                </Text>
              </View>
            </View>
          )}
          ...
          <Text style={styles.fieldLabel}>
            {isVaccine ? "টিকার নাম *" : "ওষুধের নাম / কারণ *"}
          </Text>
          <View style={styles.inputRow}>
            <Ionicons
              name={isVaccine ? "shield-checkmark-outline" : "medkit-outline"}
              size={18}
            ...
            <TextInput
              ...
              placeholder={
                isVaccine
                  ? "যেমন: FMD vaccine, HS vaccine"
                  : "যেমন: জ্বর, পেটের সমস্যা"
              }
            />
          </View>

          <Text style={styles.fieldLabel}>
            {isVaccine ? "প্রদানের তারিখ *" : "তারিখ *"}
          </Text>
          <View style={styles.inputRow}>
            <Ionicons
          ...
          {isVaccine && (
            <>
              ...
              <Text style={styles.inputHint}>
                ⚠️ তারিখ দিলে সময়মতো reminder পাবেন
              </Text>
            </>
          )}
...
function CowHealthCard({
  ...
  return (
    <View style={styles.healthCard}>
      ...
      >
        <ScoreRing
          score={cow.health_score || 0}
          grade={cow.healthGrade || "অজানা"}
        />
      ...
      {expanded && (
        <View style={styles.healthExpanded}>
          ...
          <ScoreBar
            label="💉 টিকা"
            value={details.vaccination || 0}
            max={30}
            color={Colors.success}
          />
          <ScoreBar
            label="💊 ওষুধের ইতিহাস"
            value={details.medicine || 0}
            max={25}
            color={Colors.info}
          />
          <ScoreBar
          ...
          {cow.vaccines?.length > 0 && (
            <>
              <Text style={[styles.expandedTitle, { marginTop: Spacing.lg }]}>
                টিকার ইতিহাস
              </Text>
              {cow.vaccines.map((v, i) => {
                ...
                return (
                  <View key={i} style={styles.historyRow}>
                    ...
                    {isOverdue && (
                      <Text style={styles.overdueTag}>মেয়াদ শেষ</Text>
                    )}
                  </View>
                );
              })}
...
export default function HealthTrackScreen() {
      ...
      const cowsWithHealth = await Promise.all(
        cowsList.map(async (cow) => {
          const [vaccineSnap, medicineSnap] = await Promise.all([
            getDocs(
              query(collection(db, "vaccines"), where("cow_id", "==", cow.id)),
            ),
            getDocs(
              query(collection(db, "medicines"), where("cow_id", "==", cow.id)),
            ),
          ]);
  ...
  const avgScore = cows.length
    ? Math.round(
        cows.reduce((s, c) => s + (c.health_score || 0), 0) / cows.length,
      )
    : 0;
      ...
      >
        ...
        <View style={styles.summaryBox}>
          ...
          <View style={styles.summaryRight}>
            ...
            {cows.length === 0 && (
              <Text style={styles.gradeCount}>কোনো গরু নেই</Text>
            )}
          </View>
        </View>
      </LinearGradient>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <CowHealthCard
              ...
              onAddVaccine={(c) => {
                setModalCow(c);
                setModalMode("vaccine");
              }}
              onAddMedicine={(c) => {
                setModalCow(c);
                setModalMode("medicine");
              }}
            />
          )}
          ListEmptyComponent={
