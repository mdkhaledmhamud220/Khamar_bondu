...
const fmtS = (n) => `${n >= 0 ? '+' : '-'}৳${Math.abs(n || 0).toLocaleString('bn-BD')}`;
...
function CowProfitRow({ cow, onSell }) {
  const totalCost = (cow.purchaseCost || 0) + (cow.feedCost || 0) + (cow.medicineCost || 0) + (cow.otherCost || 0);
  const isSold    = cow.status === 'sold';
  const profit    = isSold ? (cow.profit || 0) : (cow.price || 0) - totalCost;
  const profitColor = profit >= 0 ? Colors.success : Colors.error;

  return (
    <View style={styles.cowRow}>
      <View style={styles.cowRowLeft}>
        <View style={styles.cowEmoji}><Text style={{ fontSize: 20 }}>🐄</Text></View>
        <View style={styles.cowRowInfo}>
          ...
          <View style={styles.cowRowMeta}>
            ...
            <Text style={styles.metaText}>{isSold ? `বিক্রি: ${fmt(cow.salePrice)}` : `মূল্য: ${fmt(cow.price)}`}</Text>
          </View>
        </View>
      </View>
      <View style={styles.cowRowRight}>
        <Text style={[styles.cowProfit, { color: profitColor }]}>{fmtS(profit)}</Text>
        {!isSold && (
...
export default function ProfitScreen() {
  ...
  //   try {
  //     const [cowsRes, dashRes] = await Promise.all([
  //       api.get('/cows', { params: { farmerId: 'me' } }),
  //       api.get('/farm/dashboard'),
  //     ]);
  //     setCows(cowsRes.data.data || []);
  ...
  const loadData = useCallback(async () => {
    ...
    const mockDashboard = {
      ...
      totalExpectedValue: 200000,
      milkIncomeLastMonth: 9500,
  ...
  // const handleSellConfirm = async (cowId, salePrice, buyerName) => {
  //   if (!salePrice || salePrice <= 0) {
  //     Alert.alert('ত্রুটি', 'বিক্রয় মূল্য দিন।');
  //     return;
  //   }
  //   try {
  //     await api.post(`/farm/sell/${cowId}`, { salePrice, buyerName });
  //     Alert.alert('✅ বিক্রি সম্পন্ন', 'গরু বিক্রির তথ্য সংরক্ষিত হয়েছে।');
  //     setSellingCow(null);
  //     loadData();
  //   } catch (e) {
  //     Alert.alert('ত্রুটি', e.userMessage || 'বিক্রি নিবন্ধন করা যায়নি।');
  //   }
  // };
  const handleSellConfirm = (cowId, salePrice, buyerName) => {
    const updated = cows.map(c => {
      if (c.id === cowId) {
        const totalCost = c.purchaseCost + c.feedCost + c.medicineCost + c.otherCost;
        return {
          ...c,
          status: 'sold',
          salePrice,
  ...
  return (
    <View style={styles.container}>
      {/* Header */}
      <LinearGradient colors={['#1A237E', '#283593']} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
        ...
        <View style={styles.heroBox}>
          ...
          <Text style={[styles.heroValue, { color: totalProfit >= 0 ? '#A5D6A7' : '#EF9A9A' }]}>
            {fmtS(totalProfit)}
          </Text>
          <View style={styles.heroRow}>
            <Text style={styles.heroSub}>বিনিয়োগ: {fmt(investment)}</Text>
            <Text style={styles.heroDot}>•</Text>
            <Text style={styles.heroSub}>আনুমানিক মূল্য: {fmt(expectedValue)}</Text>
          </View>
        </View>
      </LinearGradient>
          ...
          <>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.statsCol}>
                <StatCard icon="cash-outline"        label="বিক্রি থেকে আয়"       value={fmtS(soldProfit)}  color={soldProfit >= 0 ? Colors.success : Colors.error} sub={`${soldCows.length}টি গরু বিক্রি`} />
                <StatCard icon="water-outline"       label="দুধ থেকে আয় (৩০দিন)" value={fmt(milkIncome)}    color="#1565C0" />
                <StatCard icon="trending-up-outline" label="আনুমানিক মুনাফা"       value={fmtS(totalProfit)} color={totalProfit >= 0 ? Colors.success : Colors.error} sub="অবিক্রীত গরু থেকে" />
                <StatCard icon="wallet-outline"      label="মোট বিনিয়োগ"           value={fmt(investment)}   color="#6A1B9A" />
              </View>
            </View>
            ...
            <View style={styles.tipBox}>
              ...
              <Text style={styles.tipText}>
                মুনাফা সঠিক দেখাতে গরু যোগ করার সময় ক্রয় মূল্য এবং নিয়মিত খরচ (খাবার, ওষুধ) আপডেট রাখুন।
              </Text>
            </View>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  statCard:   { backgroundColor: Colors.white, borderRadius: BorderRadius.lg, padding: Spacing.md, flexDirection: 'row', alignItems: 'center', gap: Spacing.md, ...Shadow.sm },
  statIcon:   { width: 44, height: 44, borderRadius: BorderRadius.md, alignItems: 'center', justifyContent: 'center' },
  statInfo:   { flex: 1 },
  ...
  cowList:    { backgroundColor: Colors.white, borderRadius: BorderRadius.lg, overflow: 'hidden', ...Shadow.sm },
  cowRow:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: Spacing.md, borderBottomWidth: 1, borderBottomColor: Colors.border },
  cowRowLeft: { flexDirection: 'row', alignItems: 'center', flex: 1, gap: Spacing.sm },
  cowEmoji:   { width: 40, height: 40, borderRadius: 20, backgroundColor: Colors.accentPale, alignItems: 'center', justifyContent: 'center' },
  cowRowInfo: { flex: 1 },
  cowRowName: { fontSize: FontSize.md, fontWeight: '700', color: Colors.textPrimary },
  cowRowStatus:{ fontSize: FontSize.xs, color: Colors.textMuted },
  cowRowMeta: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 },
  metaText:   { fontSize: FontSize.xs, color: Colors.textMuted },
  metaDot:    { color: Colors.textMuted },
  cowRowRight:{ alignItems: 'flex-end', gap: 6 },
  cowProfit:  { fontSize: FontSize.lg, fontWeight: '800' },
  sellBtn:    { backgroundColor: Colors.primary, paddingHorizontal: 12, paddingVertical: 5, borderRadius: BorderRadius.full },
  sellBtnText:{ color: Colors.white, fontSize: FontSize.xs, fontWeight: '700' },
  ...
  // Sell overlay
  sellOverlay:    { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', padding: Spacing.xl },
  sellModal:      { backgroundColor: Colors.white, borderRadius: BorderRadius.xl, padding: Spacing.xl, width: '100%' },
  sellModalTitle: { fontSize: FontSize.xl, fontWeight: '800', color: Colors.textPrimary, marginBottom: 4 },
  sellModalCow:   { fontSize: FontSize.md, color: Colors.textMuted, marginBottom: Spacing.lg },
  sellFieldLabel: { fontSize: FontSize.sm, fontWeight: '600', color: Colors.textSecondary, marginBottom: 6, marginTop: Spacing.sm },
  sellInput:      { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: Colors.border, borderRadius: BorderRadius.md, paddingHorizontal: Spacing.md, height: 48, marginBottom: 4 },
  ...
  sellCancelBtn:  { flex: 1, height: 48, borderRadius: BorderRadius.md, borderWidth: 1.5, borderColor: Colors.border, alignItems: 'center', justifyContent: 'center' },
  sellCancelText: { color: Colors.textSecondary, fontWeight: '600' },
  sellConfirmBtn: { flex: 2, height: 48, borderRadius: BorderRadius.md, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center' },
