      ...
      await Promise.all(
        selectedIds.map((id) =>
          addDoc(collection(db, "costs"), {
            ...
            note:
              note ||
              (type === "feed" ? `খাবার — ${feedGroup} গ্রুপ` : undefined),
          }),
        ),
      );
      Alert.alert(
      ...
      <View style={styles.modal}>
        ...
        >
                ...
                {FEED_GROUPS.map((g) => (
                  ...
                  >
                    ...
                    <View
                      style={[
                        styles.groupCount,
                        feedGroup === g.key && { backgroundColor: g.color },
                      ]}
                    >
                      <Text
                        style={[
                          styles.groupCountText,
                          feedGroup === g.key && { color: "#fff" },
                        ]}
                      >
                        {g.key === "বাছুর"
                    ...
                    {feedGroup === g.key && (
                      <View
                        style={[
                          styles.typeCheck,
                          { backgroundColor: g.color, top: 5, right: 5 },
                        ]}
                      >
                ...
                {filteredCows.length > 0 && (
                  ...
                  >
                    <Ionicons
                      ...
                      size={16}
                      color={
                        allFilteredSelected ? Colors.primary : Colors.textMuted
                      }
                    />
                    <Text
                      style={[
                        styles.selectAllText,
                        allFilteredSelected && { color: Colors.primary },
                      ]}
                    >
                  ...
                  {filteredCows.map((c) => {
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.cowGridCard,
                          selected && styles.cowGridCardActive,
                        ]}
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.cowGridName,
                            selected && styles.cowGridNameActive,
                          ]}
                          numberOfLines={1}
                        >
                          {c.name || c.breed}
          ...
          <View style={styles.inputRow}>
            <Text style={[styles.typeEmojiSm, { color: ct.color }]}>
              {ct.emoji}
            </Text>
            <TextInput
              style={styles.input}
...
function CostCard({ cost, cowName }) {
  ...
  return (
    <View style={styles.costCard}>
      ...
      <Text style={[styles.costAmount, { color: ct.color }]}>
        {fmt(cost.amount)}
      </Text>
    </View>
  );
...
export default function AddCostScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setCows(myCows.filter((cow) => cow.status !== "sold"));
      ...
      const allCosts = costSnapshots
        .flatMap((snapshot, index) => {
          ...
          return snapshot.docs.map((doc) =>
            normalizeCost({ id: doc.id, ...doc.data() }, cow.name || cow.breed),
          );
        })
        .sort((a, b) => {
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          showsVerticalScrollIndicator={false}
          renderItem={({ item }) => (
            <CostCard cost={item} cowName={item.cowName} />
          )}
          ListEmptyComponent={
          ...
          ListHeaderComponent={
            costs.length > 0 ? (
              <Text style={styles.listHeader}>
                সর্বশেষ {costs.length}টি লেনদেন
              </Text>
            ) : null
          }
        />
